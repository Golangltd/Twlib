...
const (
	...
	appHostIDBits int = 4 // 最大节点数 2^4 16
	sequenceBits  int = 6 // 最大毫秒并发数量 2^6 64
)
...
// 初始化ID生成器,此方法生成的id最多可以使用3年，3后仍然可以产生ID，只是客户端的lua解析之后会丢失精度，如果客户端解决了此问题，仍然可以继续使用
func InitGenerator(appID int64) error {
	...
	appHostID = appID
	startTime = stringToUnixMillionTime(timeTemplate, "2020-10-27 00:00:00") // 开始使用时间，使用之后，禁止变更，否则会出现重复ID
	return nil
}
